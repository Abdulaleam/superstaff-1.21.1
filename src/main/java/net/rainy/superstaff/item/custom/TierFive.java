            ...
            SnowballEntity snowball = new SnowballEntity(world, user) {
                ...
                protected void onBlockHit(BlockHitResult hit) {
                    ...
                    for (int x = -8; x <= 11; x++) {
                        for (int y = -3; y <= 3; y++) {
                            for (int z = -10; z <= 12; z++) {
                                // im trying to make the ridius abit not perfect square

                                BlockPos pos = center.add(x, y, z);

                                if (!world.getBlockState(pos).isAir()) {world.setBlockState(pos, Blocks.ICE.getDefaultState());
                                }}}}
